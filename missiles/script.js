...
function init() {
    ...
    const missileGeometry = new THREE.SphereGeometry(.15, 8, 8);
...
function render() {
    ...
    if (displayStats) {
        ...
        if (latest > statsDelay) {
            latest -= statsDelay;
            statsToDisplay.text = `${Math.floor(elapsed)}s | ${renderer.info.render.triangles}tri | ${fps}FPS | PixelRatio:${Math.round(window.devicePixelRatio*100)/100} | HP:${hp}`;
        }
    }
}
